...
\section{neBEM}
The nearly exact Boundary Element Method (neBEM) technique discretizes
the boundary of the domain in triangular and rectangular elements 
and solves for the charge density distribution on each of these 
boundary elements.
...
the normal component of the displacement field $\mathbf{D} = \varepsilon\mathbf{E}$ is required to be continuous,
\begin{equation*}
...
\end{lstlisting}
\begin{description}
\item[length] target linear size of the elements, measured along their edges.
\item[nmin,nmax] smallest and largest number of elements produced along either axis of a single primitive.
\end{description}
neBEM subsequently sets up the influence matrix and right-hand side vector based on the boundary conditions and material properties applied to the 
solids, and inverts the matrix to calculate the charge densities of 
each element.
