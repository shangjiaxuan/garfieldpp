...
\begin{enumerate}
  ...
  \begin{itemize}
    \item
      $\boldsymbol{\Delta{v}}_{\text{I}} = \sum\limits_{k=0}^{2}C_{\text{I}, k}\mathbf{v}_{d}\left(\mathbf{x}_{k}\right)$, accurate to second order,
    \item
      $\boldsymbol{\Delta{v}}_{\text{II}} = \sum\limits_{k=0}^{3}C_{\text{I}, k}\mathbf{v}_{d}\left(\mathbf{x}_{k}\right)$, accurate to third order.
  \end{itemize}
  These two estimates are based on the drift velocity at the starting point 
  and the velocity at only three new locations
  \begin{equation*} 
...
\begin{lstlisting}
void EnableRejectKinks(const bool on = true);
\end{lstlisting} 
...
a wire takes over. To be precise, the stepping towards the wire starts when 
the particle crosses a disk of radius $n$ times the wire radius. The factor $n$ 
...
\begin{equation*}
 \overline{n}P_{n} = \frac{\left(\theta + 1\right)^{\theta + 1}}{\Gamma\left(\theta + 1\right)}\left(\frac{n}{\overline{n}}\right)\text{e}^{-\left(\theta + 1\right)n/\overline{n}},
\end{equation*}
...
path segments projected on the local drift velocity vector. 
...
according to the local electric (and optionally magnetic) field. The duration 
$\Delta{t}$ of a free-flight step is sampled using the ``null-collision'' method 
\cite{Skullerud1968}. 
After the step, the scattering process to take place is sampled based on the 
relative collision rates at the new energy $\epsilon'$, the energy and direction 
of the electron are updated according to the type of collision.
