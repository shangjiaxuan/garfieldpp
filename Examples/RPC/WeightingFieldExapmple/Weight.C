...
int main(int argc, char * argv[]) {
    ...
    Garfield::SolidBox box(0, 0.5 * gap, 0, gap, 0.5 * gap, gap); geo.AddSolid(&box, &si);
    //...
    Garfield::ComponentUser cmp;
    cmp.SetGeometry(&geo);
    // Set the function to be called for calculating the drift field. cmp.SetElectricField(efield);
    // Set the function to be called for calculating the weighting field. cmp.SetWeightingField(wfield);
    ...
    drift.DisableDiffusion(); drift.EnableSignalCalculation();
    // Simulate an electron-hole pair starting at y = 150 um. drift.DriftElectron(0, 150.e-4, 0, 0);
    drift.DriftHole(0, 150.e-4, 0, 0);
    
    // -----
    
    // Set the function for calculating the delayed weighting field. 3 cmp.SetDelayedWeightingField(dwfield);
    //...
    ...
    Garfield::ComponentVoxel cmp; 7 cmp.SetMesh(nX, nY, 1, xMin, xMax, yMin, yMax, zMin, zMax);
    cmp.LoadElectricField("Efield.txt", "XY", false, false); cpmp.LoadWeightingField("Weighting_00.txt", "XY", false); for (unsigned int i = 0; i < nTimes; ++i) {
        char filename[50];
        sprintf(filename, "Weighting_%02d.txt", i + 1); cmp.LoadWeightingField(filename, "XY", times[i], false); 3
    ...
    // Simulate a track at perpendicular incidence. track.NewTrack(0, 0, 0, 0, 0, 1, 0);
    double xc = 0., yc = 0., zc = 0., tc = 0., ec = 0., extra = 0.; int nc = 0;
    while (track.GetCluster(xc, yc, zc, tc, nc, ec, extra)) {
        for (int i = 0; i < nc; ++i) {
            double xe = 0., ye = 0., ze = 0., te = 0., ee = 0.; double dx = 0., dy = 0., dz = 0.; track.GetElectron(i, xe, ye, ze, te, ee, dx, dy, dz); drift.DriftElectron(xe, ye, ze, te); drift.DriftHole(xe, ye, ze, te);
