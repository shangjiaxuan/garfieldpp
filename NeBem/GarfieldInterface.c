...
NewModel = 1; NewMesh = 1; NewBC = 1; NewPP = 1;
ModelCntr = 1; MeshCntr = 1; BCCntr = 1; PPCntr = 1;
...
OptGnuplot = 0; OptGnuplotPrimitives = 0; OptGnuplotElements = 0;
...
OptGnuplot = 1; OptGnuplotPrimitives = 1; OptGnuplotElements = 1;
OptPrimitiveFiles = 1; OptElementFiles = 1;
...
// -------------------------------------------------------------------
void bempar_(int* bemmin, int* bemmax, double* bemtarget, int* newflag, int* storeinverted, int* debuglevel, int* inversionmethod) {
  ...
  printf("        Element count range: %d to %d\n", MinNbElementsOnLength, MaxNbElementsOnLength);
...
void bemdis_(int* ifail) {
  ...
  for (int prim = 1; prim <= NbPrimitives; ++prim) {
    ...
    if (nb1 < MinNbElementsOnLength) {
      // printf("bemdis: Primitive %d discretisation 1 increased from %d to minimum allowed (%d).\n", prim, nb1, MinNbElementsOnLength);
      nb1 = MinNbElementsOnLength;
    } else if (nb1 > MaxNbElementsOnLength) {
      printf("bemdis: Primitive %d discretisation 1 decreased from %d to maximum allowed (%d).\n", prim, nb1, MaxNbElementsOnLength);
	...
	// printf("bemdis: Primitive %d discretisation 2 increased from %d to minimum allowed (%d).\n", prim, nb2, MinNbElementsOnLength);
	nb2 = MinNbElementsOnLength;
      } else if (nb2 > MaxNbElementsOnLength) {
	printf("bemdis: Primitive %d discretisation 2 decreased from %d to maximum allowed (%d).\n", prim, nb2, MaxNbElementsOnLength);
    ...
    // printf("bemdis: Primitive %d discretisation size = %g, primitive size l1 = %g, l2 = %g, n1 = %d, n2 = %d.\n",
	   // prim, size, sqrt(l1), sqrt(l2), nb1, nb2);
...
void bemdlw_(int* id) {
  neBEMDeleteWeightingField(*id);
}

// -------------------------------------------------------------------

void ionbem_(float* x, float* y, float* z,
...
void efcbem_(float* x, float* y, float* z,
             float* ex, float* ey, float* ez, float* v,
             int* opt, int* loc) {
  // Default value
  *ex = 0;
  *ey = 0;
  ...
  *loc = 0;
  ...
  if (ivol > -1) {
    ...
    *loc = 2*MXWIRE + ivol+1;
    ...
    neBEMVolumeDescription(ivol, &shape,
			   &material, &epsilon,
			   &potential, &charge,
			   &boundarytype);
    if (boundarytype == 1) {
  ...
  //  printf("\tField = (%g,%g,%g) V/m, V = %g V\n", field.X, field.Y, field.Z, potential);
  // Convert units and precision
  *ex = (float) (field.X)/100.0;
  *ey = (float) (field.Y)/100.0;
  *ez = (float) (field.Z)/100.0;
...
void bemvoq_(int* volume, double* q) {
 *q = neBEMVolumeCharge(*volume);
}
...
int neBEMGetState(void) {
  return neBEMState;
}

// -------------------------------------------------------------------

void bemqst_(int* status) {
  *status = neBEMGetState();
}
