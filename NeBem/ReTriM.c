...
int SurfaceElements(int prim, int nvertex,
...
switch(nvertex)
	{
	case 3:	// triangle
		fstatus = DiscretizeTriangle(prim, nvertex, xvert, yvert, zvert,
							xnorm, ynorm, znorm, volref1, volref2,
							inttype, potential, charge, lambda,
							NbSegX, NbSegZ);
		// assert(fstatus == 0);
		if(fstatus != 0)
			{
	...
	case 4:	// rectangle
		fstatus = DiscretizeRectangle(prim, nvertex, xvert, yvert, zvert,
							xnorm, ynorm, znorm, volref1, volref2,
							inttype, potential, charge, lambda,
                            NbSegX, NbSegZ);
		// assert(fstatus == 0);
...
int WireElements(int prim, int nvertex,
									double xvert[], double yvert[], double zvert[],
									double radius, 
									int volref1, int volref2,
									int inttype, double potential, double charge,
...
switch(nvertex)
	{
	case 2:	// wire
		fstatus = DiscretizeWire(prim, nvertex, xvert, yvert, zvert,
																	radius, volref1, volref2,
																	inttype, potential, charge, lambda,
																	NbSegs);
		// assert(fstatus == 0);
		if(fstatus != 0)
...
if(nb < 1)	// absurd! use the trio: target, min, max
	...
	if( (nb > MinNbElementsOnLength) && (nb < MaxNbElementsOnLength) )
		{	// nothing to be done
		}
	...
	// Need to have at least MinNbElementsOnLength elements per wire primitive
		...
		if(ellength < MINDIST)	// which may not be possible if the length is small
			{
	...
	*NbSeg = nb;
	...
	{	// number of dicretization specified by user
	double lWire = (XVertex[prim][1]-XVertex[prim][0])
										*(XVertex[prim][1]-XVertex[prim][0])
									+ (YVertex[prim][1]-YVertex[prim][0])
										*(YVertex[prim][1]-YVertex[prim][0])
	...
	*NbSegCoord2 = nb2;
	...
	fprintf(fMeshLog,
									"Number of elements on surface primitive %d is %d X %d.\n\n",
											prim, *NbSegCoord1, *NbSegCoord2);
	}
else
	{	// number of discretization specified by the user
	...
	if(l1 > l2)
		...
		if(nb2 > nb1)	// swap numbers to allow larger number to larger side
			{
			int tmpnb = nb1; nb1 = nb2; nb2 = tmpnb;
			}
		}	// if l1 > l2
...
int DiscretizeWire(int prim, int nvertex,
										double xvert[], double yvert[], double zvert[],
										double radius,
										int volref1, int volref2, 
										int inttype, double potential, double charge,
...
if(OptPrintVertexAndNormal)
	...
	for(register int vert=0; vert<nvertex; ++vert)
		...
		printf("vert: %d, x: %lg, y: %lg, z: %lg\n",
						vert, xvert[vert], yvert[vert], zvert[vert]);
		}
	printf("radius: %lg\n", radius);
	}	// if OptPrintVertexAndNormal
...
fPrim = NULL; fElem = NULL; fgpElem = NULL;
...
if(OptPrimitiveFiles)
	{
	char OutPrim[256];
	strcpy(OutPrim, ModelOutDir); strcat(OutPrim, "/Primitives/Primitive");
	strcat(OutPrim, primstr); strcat(OutPrim, ".out");
...
if(OptGnuplot && OptGnuplotPrimitives)
	{
	char gpPrim[256];
	strcpy(gpPrim, MeshOutDir); strcat(gpPrim, "/GViewDir/gpPrim");
	strcat(gpPrim, primstr); strcat(gpPrim, ".out");
...
if(OptElementFiles)
	{
	char OutElem[256];
	strcpy(OutElem, MeshOutDir); strcat(OutElem, "/Elements/ElemOnPrim");
	strcat(OutElem, primstr); strcat(OutElem, ".out");
...
if(OptGnuplot && OptGnuplotElements)
	{
	strcpy(gpElem, MeshOutDir); strcat(gpElem, "/GViewDir/gpElemOnPrim");
	strcat(gpElem, primstr); strcat(gpElem, ".out");
...
for(register int seg = 1; seg <= NbSegs; ++seg)
	...
	(EleArr+EleCntr-1)->DeviceNb = 1;	// At present, there is only one device
	...
	(EleArr+EleCntr-1)->BC.NbOfBCs = 1; // assume one BC per element
 	(EleArr+EleCntr-1)->BC.CollPt.X = (EleArr+EleCntr-1)->G.Origin.X;	// modify
 	(EleArr+EleCntr-1)->BC.CollPt.Y = (EleArr+EleCntr-1)->G.Origin.Y;	// to be on
 	(EleArr+EleCntr-1)->BC.CollPt.Z = (EleArr+EleCntr-1)->G.Origin.Z;	// surface?
	...
	if(OptElementFiles)
		...
		fprintf(fElem, "%lg, %lg, %lg\n", (EleArr+EleCntr-1)->G.DC.XUnit.X,
			(EleArr+EleCntr-1)->G.DC.XUnit.Y, (EleArr+EleCntr-1)->G.DC.XUnit.Z);
		fprintf(fElem, "%lg, %lg, %lg\n", (EleArr+EleCntr-1)->G.DC.YUnit.X,
			(EleArr+EleCntr-1)->G.DC.YUnit.Y, (EleArr+EleCntr-1)->G.DC.YUnit.Z);
		fprintf(fElem, "%lg, %lg, %lg\n", (EleArr+EleCntr-1)->G.DC.ZUnit.X,
			(EleArr+EleCntr-1)->G.DC.ZUnit.Y, (EleArr+EleCntr-1)->G.DC.ZUnit.Z);
...
if(OptPrimitiveFiles)
	...
	fprintf(fPrim, "Element begin: %d, Element end: %d\n",
									ElementBgn[prim], ElementEnd[prim]);
...
int DiscretizeTriangle(int prim, int nvertex,
                        double xvert[], double yvert[], double zvert[],
                        double xnorm, double ynorm, double znorm,
                        int volref1, int volref2,
												int inttype, double potential, double charge,
...
// in order to avoid warning messages
fPrim = NULL; fElem = NULL; fgpMesh = NULL; fgpElem = NULL;
...
SurfX = xvert[1]; SurfY = yvert[1]; SurfZ = zvert[1];
PrimOrigin.X = SurfX; PrimOrigin.Y = SurfY; PrimOrigin.Z = SurfZ;
...
if(DebugLevel == 202)
	{
	printf("First attempt: \n"); PrintDirnCosn3D(PrimDirnCosn); printf("\n");
	}
...
if(OptPrimitiveFiles)
	...
	fprintf(fPrim, "%lg\t%lg\t%lg\t%lg\t%lg\n",
									SurfX, SurfY, SurfZ, SurfLX, SurfLZ);
	fprintf(fPrim, "#DirnCosn: \n");
	fprintf(fPrim, "%lg, %lg, %lg\n", PrimDirnCosn.XUnit.X,
									PrimDirnCosn.XUnit.Y, PrimDirnCosn.XUnit.Z);
	fprintf(fPrim, "%lg, %lg, %lg\n", PrimDirnCosn.YUnit.X,
...
if(OptGnuplot && OptGnuplotElements)
	...
	// gnuplot friendly file outputs for elements on primitive
	strcpy(gpMesh, MeshOutDir); strcat(gpMesh, "/GViewDir/gpMeshOnPrim");
	strcat(gpMesh, primstr); strcat(gpMesh, ".out");
...
// This rotated vector (now in the global system) is then added to the position
// vector of the primitive CG (always in the global system) to get the CG of
// the element in the global system.
...
for(register int k = 1; k <= NbSegZ; ++k)	// consider the k-th row
  ...
  localDisp.X = xtorigin; localDisp.Y = ytorigin; localDisp.Z = ztorigin;
  globalDisp = RotatePoint3D(&localDisp, &PrimDirnCosn, local2global);
	SurfElX = SurfX + globalDisp.X;	// these are the coords in GCS of origin of
	SurfElY = SurfY + globalDisp.Y;	// the triangluar element under consideration
	...
	(EleArr+EleCntr-1)->G.dA = 0.5 * (EleArr+EleCntr-1)->G.LX
										* (EleArr+EleCntr-1)->G.LZ;
  ...
  localDisp.X = dxl; localDisp.Y = dyl; localDisp.Z = dzl;
	if(DebugLevel == 201)
		{
  ...
  globalDisp = RotatePoint3D(&localDisp, &PrimDirnCosn, local2global);
	...
	if(DebugLevel == 201)
		...
		printf("Element global dxl, dxy, dxz: %lg %lg %lg\n",
											globalDisp.X, globalDisp.Y, globalDisp.Z);
	...
	for(register int i = 1; i <= NbSegXOnThisRow; ++i)
   	{
		...
		localDisp.X = xorigin; localDisp.Y = yorigin; localDisp.Z = zorigin;
		globalDisp = RotatePoint3D(&localDisp, &PrimDirnCosn, local2global);
		...
		(EleArr+EleCntr-1)->G.LZ = SurfElLZ;
		(EleArr+EleCntr-1)->G.LZ = zhipt - zlopt;	// to be on the safe side! 21/2/14
		(EleArr+EleCntr-1)->G.dA = (EleArr+EleCntr-1)->G.LX
										* (EleArr+EleCntr-1)->G.LZ;
		// Safe to use the direction cosines obtained for the triangular primitive
		// since they are bound to remain unchanged for the triangular sub-elements
		(EleArr+EleCntr-1)->G.DC.XUnit.X = PrimDirnCosn.XUnit.X;
		(EleArr+EleCntr-1)->G.DC.XUnit.Y = PrimDirnCosn.XUnit.Y;
		(EleArr+EleCntr-1)->G.DC.XUnit.Z = PrimDirnCosn.XUnit.Z;
		(EleArr+EleCntr-1)->G.DC.YUnit.X = PrimDirnCosn.YUnit.X;
		...
		x0 = - 0.5*(EleArr+EleCntr-1)->G.LX;	// xyz displacement of node wrt ECS
		y0 = 0.0;
		z0 = - 0.5*(EleArr+EleCntr-1)->G.LZ;
    {   // Separate block for position rotation - local2global
    Point3D localDisp, globalDisp;

    localDisp.X = x0; localDisp.Y = y0; localDisp.Z = z0;	// displacement in GCS
    globalDisp = RotatePoint3D(&localDisp, &PrimDirnCosn, local2global);
		x0 = (EleArr+EleCntr-1)->G.Origin.X + globalDisp.X;	// xyz position in GCS
    ...
    localDisp.X = x1; localDisp.Y = y1; localDisp.Z = z1;
    globalDisp = RotatePoint3D(&localDisp, &PrimDirnCosn, local2global);
    ...
    localDisp.X = x2; localDisp.Y = y2; localDisp.Z = z2;
    globalDisp = RotatePoint3D(&localDisp, &PrimDirnCosn, local2global);
    ...
    localDisp.X = x3; localDisp.Y = y3; localDisp.Z = z3;
    globalDisp = RotatePoint3D(&localDisp, &PrimDirnCosn, local2global);
...
int DiscretizeRectangle(int prim, int nvertex,
                        double xvert[], double yvert[], double zvert[],
                        double xnorm, double ynorm, double znorm,
                        int volref1, int volref2,
												int inttype, double potential, double charge,
...
for(register int i = 1; i <= NbSegX; ++i)
	{
	x1 = -SurfLX/2.0 + (double)(i-1)*SurfElLX;	// assuming centroid at 0,0,0
	...
	for(register int k = 1; k <= NbSegZ; ++k)
  	{
   	...
   	zav = 0.5*(z1+z2);
		...
		localDisp.X = xav; localDisp.Y = 0.0; localDisp.Z = zav;
		globalDisp = RotatePoint3D(&localDisp, &PrimDirnCosn, local2global);
...
int DiscretizePolygon(int prim, int nvertex,
                        double xvert[], double yvert[], double zvert[],
                        double xnorm, double ynorm, double znorm,
                        int volref1, int volref2,
												int inttype, double potential, double charge,
...
for(register unsigned int ele = 1; ele <= NbElements; ++ele)
  {
	...
	switch(PrimType[prim])	// Note this condition is pure geometry, not electric!
		{
		case 2:
