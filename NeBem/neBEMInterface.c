...
// Version information
strcpy(neBEMVersion, "1.9.07"); strcpy(ISLESVersion, "1.4.8");
printf("Using neBEM version %s and ISLES version %s\n",
				neBEMVersion, ISLESVersion);
...
char IslesFile[256];
strcpy(IslesFile, PPOutDir); strcat(IslesFile, "/Isles.log");
...
if(mapInpFile == NULL)
	...
	{	// read first since that will decide whether there is a map and its version.
...
if(fixedWtInpFile == NULL)
	{
	printf("neBEMFixedWtField.inp absent ... assuming OptFixedWtField = 0 ...\n");
	OptFixedWtField = 0;
...
if(fastWtFldInpFile == NULL)
	{
	printf("neBEMWtFldFastVol.inp absent ... assuming OptWtFldFastVol = 0 ...\n");
	OptWtFldFastVol = 0;
...
NbSurfs = 0; NbWires = 0; neBEMState = 2;
...
for(int prim = 1; prim <= NbPrimitives; ++prim)
  ...
  fstatus = neBEMGetPrimitive(prim, &nvertex,
	...
	if(volmax < volref1) {volmax = volref1;}	// maxm nb of volumes
	if(volmax < volref2) {volmax = volref2;}	// maxm nb of volumes
    ...
    for (int ivertex=0; ivertex<nvertex; ivertex++)
			{
     	printf("\tnode %d (%g,%g,%g)\n", ivertex,
     	    		xvert[ivertex], yvert[ivertex], zvert[ivertex]);
    	}
	...
	if(volref1 == -1)	// Must be in error, since no device is made of vacuum
		...
		{
  	neBEMVolumeDescription(volref1, &shape1,
               						&material1, &epsilon1,
               						&potential1, &charge1,
               						&boundarytype1);
		}
	...
	if(volref2 == -1)
		{
		shape2 = 0; material2 = 11; epsilon2 = 1.0; potential2 = 0.0;
		charge2 = 0.0; boundarytype2 = 0;
		}
	else
		{
	  neBEMVolumeDescription(volref2, &shape2,
               						&material2, &epsilon2,
               						&potential2, &charge2,
               						&boundarytype2);
		}
	if(OptPrintVolumeDetails)
		...
		printf("\t\tepsilon2: %lg, potential2: %lg, charge2: %lg\n",
						epsilon2, potential2, charge2);
		}

// Put default values to variables that depend on the interface type
// Is there any risk involved in putting these defaults?
// At present, they even seem necessary. For example, for floating conductors
// or dielectric-dielectric interface, the formulation requires that the RHS
// is zero (may be modified by the effect of known charges).
	Epsilon1[prim] = epsilon1; Epsilon2[prim] = epsilon2;	// 1: self, 2: external
	ApplPot[prim] = 0.0; Lambda[prim] = 0.0; ApplCh[prim] = 0.0;
...
// THE JOURNAL OF CHEMICAL PHYSICS 130, 094102 (2009)
  ...
  switch(boundarytype1)
		{ // the volume itself is volref1
     	...
     	if (boundarytype2 == 0 || boundarytype2 == 4)	// dielectric-conductor
    		...
    		if (fabs(potential1-potential2)	// same potential
      		...
      		return -1;
				...
				{
    		printf("neBEMReadGeometry: conductor at given potential; rejected.\n");
    		return -1;
      	}
      break;

    case 2:     // conductor with a specified charge
      if ( (boundarytype2 == 0) || (boundarytype2 == 4) )// conductor-dielectric
				{
    ...
    case 4: // dielectric intrface (plastic-plastic) without "manual" charge
    	if (boundarytype2 == 0)							// dielectric-vacuum
    		...
    		if (fabs(epsilon1-epsilon2)				// identical dielectrica
					...
					{
     			printf("neBEMReadGeometry: between identical dielectrica; skipd.\n");
					printf("Primitive skipped: #%d\n", prim);
     			InterfaceType[prim] = 0;
    			}
    ...
    case 5: // dielectric with surface charge (plastic-gas, typically)
    	if (boundarytype2 == 0)							// dielectric-vacuum
    		...
    		if (fabs(epsilon1-epsilon2)				// identical dielectrica
				...
				{
    		printf("neBEMReadGeometry: charged dielectric adjacent to a conductor; rejected.\n");
    		return -1;
      	}
      break;
	...
	if(OptPrintVolumeDetails)
		{
		printf("\tType: %d, ApplPot: %lg, Epsilon1: %lg, Epsilon2: %lg, Lambda: %lg, ApplCh: %lg\n",
							InterfaceType[prim], ApplPot[prim], Epsilon1[prim], Epsilon2[prim], Lambda[prim], ApplCh[prim]);
		}
	...
	int jx, jy, jz;
	double sx, sy, sz;
	fstatus = neBEMGetPeriodicities(prim, &ix, &jx, &sx, &iy, &jy, &sy,
																				&iz, &jz, &sz);
	if(fstatus != 0)
		{
	...
	PeriodicTypeX[prim] = ix; PeriodicTypeY[prim] = iy; PeriodicTypeZ[prim] = iz;
	if (0)
		{
	...
	if(ix>0)	// These checks need to be done separately. Otherwise, there is
  ...
  double sx, sy, sz;
  fstatus = neBEMGetMirror(prim, &ix, &jx, &sx, &iy, &jy, &sy, &iz, &jz, &sz);
  if(fstatus != 0)
    {
  ...
  MirrorTypeX[prim] = ix; MirrorTypeY[prim] = iy; MirrorTypeZ[prim] = iz;
  if (0)
    {
    printf("For primitive: %d\n", prim);
  ...
  if (ix>0)
    ...
    {
    MirrorDistXFromOrigin[prim] = 0.0;  // assumed to pass through the origin
    }
  if (iy>0)
    {
    // printf("neBEMReadGeometry: Mirror have been requested.\n");
  ...
  if (iz>0)
	...
	fstatus = neBEMGetBoundingPlanes(&ixmin, &cxmin, &vxmin,
         														&ixmax, &cxmax, &vxmax,
         														&iymin, &cymin, &vymin,
         														&iymax, &cymax, &vymax,
         														&izmin, &czmin, &vzmin,
	...
	BndPlaneInXMin[prim] = ixmin; BndPlaneInXMax[prim] = ixmax;
	BndPlaneInYMin[prim] = iymin; BndPlaneInYMax[prim] = iymax;
	BndPlaneInZMin[prim] = izmin; BndPlaneInZMax[prim] = izmax;
	if(ixmin)
		{
		XBndPlaneInXMin[prim] = cxmin; VBndPlaneInXMin[prim] = vxmin;
		}
	else
		{
		XBndPlaneInXMin[prim] = 0.0; VBndPlaneInXMin[prim] = 0.0;
		}
	if(ixmax > 0)
		{
		XBndPlaneInXMax[prim] = cxmax; VBndPlaneInXMax[prim] = vxmax;
		}
	else
		{
		XBndPlaneInXMax[prim] = 0.0; VBndPlaneInXMax[prim] = 0.0;
		}
	if(iymin > 0)
		{
		YBndPlaneInYMin[prim] = cymin; VBndPlaneInYMin[prim] = vymin;
		}
	else
		{
		YBndPlaneInYMin[prim] = 0.0; VBndPlaneInYMin[prim] = 0.0;
		}
	if(iymax > 0)
		{
		YBndPlaneInYMax[prim] = cymax; VBndPlaneInYMax[prim] = vymax;
		}
	else
		{
		YBndPlaneInYMax[prim] = 0.0; VBndPlaneInYMax[prim] = 0.0;
		}
	if(izmin > 0)
		{
		ZBndPlaneInZMin[prim] = czmin; VBndPlaneInZMin[prim] = vzmin;
		}
	else
		{
		ZBndPlaneInZMin[prim] = 0.0; VBndPlaneInZMin[prim] = 0.0;
		}
	if(izmax > 0)
		{
		ZBndPlaneInZMax[prim] = czmax; VBndPlaneInZMax[prim] = vzmax;
		}
	else
		{
		ZBndPlaneInZMax[prim] = 0.0; VBndPlaneInZMax[prim] = 0.0;
		}
	}	// loop over the primitives - major loop
...
for(int volref = 0; volref <= VolMax; ++volref)
	{
 	neBEMVolumeDescription(volref, &volShape[volref],
               						&volMaterial[volref], &volEpsilon[volref],
               						&volPotential[volref], &volCharge[volref],
               						&volBoundaryType[volref]);
	if(dbgFn)
		{
		printf("volref: %d\n", volref);
		printf("shape: %d,  material: %d\n", volShape[volref], volMaterial[volref]);
...
// Ideally, all the removal conditions for a primitive should be checked in one
// loop and the list should be updated in one single go.
{
for(int prim = 1; prim <= NbPrimitives; ++prim)
	{
...
for(int prim = 1; prim <= NbPrimitives; ++prim)
	...
	for(int vert = 0; vert < NbVertices[prim]-1; ++vert)
		{
		DVertex[vert] = sqrt( ((XVertex[prim][vert+1] - XVertex[prim][vert])
...
// 3. Find out their positions after one positive repetition in both X and Y.
// and check which of them fall on already existing primitives having normal
// along X.
// A. Similarly for one negative repetition.
// II. Same check for all the primitives having normal along Y will have to be
// carried out.
// Similar approach could be useful for an arbitrary primitive if we work in
// the local coordinate system of the primitive and check all other primitives
// in that LCS.
// Complications are likely to arise due to corner and edge overlaps which will
// happen to all neighbouring primitives on the same plane.
int Overlap[NbPrimitives+1][NbPrimitives+1];	// this C++ syntax seems to work!
// int **Overlap;
...
for(register unsigned int prim = 1; prim <= NbPrimitives; ++prim)
	...
	if(fabs(fabs(XNorm[prim]) - 1.0) >= 1.0e-12)	// primitive || to YZ plane
		continue;
	...
	for(register unsigned int chkprim = prim+1;
		...
		if(fabs(fabs(XNorm[chkprim]) - 1.0) >= 1.0e-12)	// primitive || to YZ plane
			continue;

		if(fabs(XVertex[prim][0] - XVertex[chkprim][0]) <= 1.0e-12)
			{	// same plane; check YZ vertices for full / partial overlap
			double smallY = YVertex[prim][0]; double bigY = YVertex[prim][0];
			double smallZ = ZVertex[prim][0]; double bigZ = ZVertex[prim][0];
			for(register unsigned int vert = 1; vert<= NbVertices[prim]; ++vert)
				{
			...
			// Because of the rearrangement, the big and small vertices are now
			// distinct and unambiguous. Comparisons can now be made a lot more
			// easily.
			// One important assumption is implicit,however.
			// The edges of the primitive are assumed to parallel to the
			// Y and Z axes. We are assuming rectangular primitives as a
			// consequence.
			// One to one match of all the vertices
			// Subsequent checks should be carried out only if Overlap is still 0

			// No overlap
			if(((smallcY > smallY) && (smallcY > bigY))
					&& ((bigcY > smallY) && (bigcY > bigY)))
				Overlap[prim][chkprim] = -1;	// no Y overlap
			if(Overlap[prim][chkprim] != 0) continue;
			if(((smallcZ > smallZ) && (smallcZ > bigZ))
					&& ((bigcZ > smallZ) && (bigcZ > bigZ)))
				Overlap[prim][chkprim] = -2;	// no Z overlap
			if(Overlap[prim][chkprim] != 0) continue;
			...
			if((fabs(bigcY-smallY) <= MINDIST)
					&& (((smallcZ > smallZ) && (smallcZ < bigZ))	// = implies corner
					|| ((bigcZ > smallZ) && (bigcZ < bigZ))))	// = implies corner
				Overlap[prim][chkprim] = -7;
			if(Overlap[prim][chkprim] != 0) continue;
			if((fabs(smallcZ-bigZ) <= MINDIST)
					&& (((smallcY > smallY) && (smallcY < bigY))	// = implies corner
					|| ((bigcY > smallY) && (bigcY < bigY))))	// = implies corner
				Overlap[prim][chkprim] = -8;
			if(Overlap[prim][chkprim] != 0) continue;
			if((fabs(smallcY-bigY) <= MINDIST)
					&& (((smallcZ > smallZ) && (smallcZ < bigZ))	// = implies corner
					|| ((bigcZ > smallZ) && (bigcZ < bigZ))))	// = implies corner
			...
			if((fabs(smallcY-smallY) <= MINDIST)
					...
					&& (fabs(bigcZ-bigZ) <= MINDIST))
				{	// chkprim has a one-to-one overlap with prim
				Overlap[prim][chkprim] = 1;
				}
			if(Overlap[prim][chkprim] != 0) continue;

			// checkprim is completely contained within prim - case 1
			if((smallcY > smallY) && (smallcY < bigY)
					 && (smallcZ > smallZ) && (smallcZ < bigZ)
					&& (bigcY > smallY) && (bigcY < bigY)
					 && (bigcZ > smallZ) && (bigcZ < bigZ))
				{	// chkprim completely contained within prim
			...
			if((smallY > smallcY) && (smallY < bigcY)
					 && (smallZ > smallcZ) && (smallZ < bigcZ)
					&& (bigY > smallcY) && (bigY < bigcY)
					 && (bigZ > smallcZ) && (bigZ < bigcZ))
				{	// chkprim completely contains prim
				Overlap[prim][chkprim] = 3;
				}
			if(Overlap[prim][chkprim] != 0) continue;

			// checkprim completely contained within prim in Z direction - case 3
			if((smallcZ > smallZ) && (bigcZ < bigZ))
				{
				if((smallcY > smallY) && (smallcY < bigY)
						&& (bigcY > smallY) && (bigcY > bigY))
					{	// but not Y: bigY outside prim
					Overlap[prim][chkprim] = 4;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				if((smallcY < smallY) && (smallcY < bigY)
						&& (bigcY > smallY) && (bigcY < bigY))
					{	// but not Y: smallY outside prim
					Overlap[prim][chkprim] = 5;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				if((smallcY < smallY) && (smallcY < bigY)
						&& (bigcY > smallY) && (bigcY > bigY))
					{	// but not Y: chkprim wider than prim
					Overlap[prim][chkprim] = 6;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				}

			// checkprim completely contained within prim in Y direction - case 4
			if((smallcY > smallY) && (bigcY < bigY))
				{
				if((smallcZ > smallZ) && (smallcZ < bigZ)	// bigcZ outside
						&& (bigcZ > smallZ) && (bigcZ > bigZ))
					{	// but not Z: bigZ outside prim
					Overlap[prim][chkprim] = 7;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				if((smallcZ < smallZ) && (smallcZ < bigZ)	// smallcZ outside
						&& (bigcZ > smallZ) && (bigcZ < bigZ))
					{	// but not Z: smallZ outside prim
					Overlap[prim][chkprim] = 8;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				if((smallcZ < smallZ) && (smallcZ < bigZ)	// smallcZ outside
						&& (bigcZ > smallZ) && (bigcZ > bigZ)) // bigcZ outside
					{	// but not Z: chkprim taller than prim
					Overlap[prim][chkprim] = 9;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				}

			// checkprim completely contains prim in Z direction - case 5
			if((smallcZ < smallZ) && (bigcZ > bigZ))
				{
				if((smallY > smallcY) && (smallY < bigcY)	// bigY outside
						&& (bigY > smallcY) && (bigY > bigcY))
					{	// but not Y: bigY outside chkprim
					Overlap[prim][chkprim] = 10;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				if((smallY < smallcY) && (smallY < bigcY)	// smallY outside
						&& (bigY > smallcY) && (bigY < bigcY))
					{	// but not Y: smallY outside chkprim
					Overlap[prim][chkprim] = 11;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				if((smallY < smallcY) && (bigY > bigcY))	// outside chkprim extent
					{	// but not Y: prim wider than chkprim
					Overlap[prim][chkprim] = 12;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				}

			// checkprim completely contains prim in Y direction - case 6
			if((smallcY < smallY) && (bigcY > bigY))
				{
				if((smallZ > smallcZ) && (smallZ < bigcZ)	// bigZ outside
						&& (bigZ > smallcZ) && (bigZ > bigcZ))
					{	// but not Z: bigZ outside chkprim
					Overlap[prim][chkprim] = 13;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				if((smallZ < smallcZ) && (smallZ < bigcZ)	// smallZ outside
						&& (bigZ > smallcZ) && (bigZ < bigcZ))
					{	// but not Z: smallZ outside chkprim
					Overlap[prim][chkprim] = 14;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				if((smallZ < smallcZ) && (smallZ < bigcZ)	// smallZ and bigZ outside
						&& (bigZ > smallcZ) && (bigZ < bigcZ))
					{	// but not Z: prim taller than chkprim
					Overlap[prim][chkprim] = 14;
					}
				if(Overlap[prim][chkprim] != 0) continue;
				}

			// check cases where only one corner of chkprim is within prim
			// all the other three being beyond.
			if((smallcY > smallY) && (smallcY < bigY)
					&& (smallcZ > smallZ) && (smallcZ < bigZ)
					&& (bigcY > bigY) && (bigcZ > bigZ))
				{	// only smallcY,smallcZ corner within prim
				Overlap[prim][chkprim] = 15;
				}
			if(Overlap[prim][chkprim] != 0) continue;
			if((bigcY > smallY) && (bigcY < bigY)
					&& (smallcZ > smallZ) && (smallcZ < bigZ)
					&& (smallcY < smallY) && (bigcZ > bigZ))
				{	// only bigcY,smallcZ corner within prim
				Overlap[prim][chkprim] = 16;
				}
			if(Overlap[prim][chkprim] != 0) continue;
			if((smallcY > smallY) && (smallcY < bigY)
					&& (bigcZ > smallZ) && (bigcZ < bigZ)
					&& (bigcY > bigY) && (smallcZ < smallZ))
				{	// only smallcY,bigcZ corner within prim
				Overlap[prim][chkprim] = 17;
				}
			if(Overlap[prim][chkprim] != 0) continue;
			if((bigcY > smallY) && (bigcY < bigY)
					&& (bigcZ > smallZ) && (bigcZ < bigZ)
					&& (smallcY < smallY) && (smallcZ < smallZ))
				{	// only bigcY,bigcZ corner within prim
				Overlap[prim][chkprim] = 18;
				}
			if(Overlap[prim][chkprim] != 0) continue;

			// check cases where only one corner of prim is within chkprim
			// all the other three being beyond.
			if((smallY > smallcY) && (smallY < bigcY)
					&& (smallZ > smallcZ) && (smallZ < bigcZ)
					&& (bigcY < bigY) && (bigcZ < bigZ))
			...
			if((bigY > smallcY) && (bigY < bigcY)
					&& (smallZ > smallcZ) && (smallZ < bigcZ)
					&& (smallcY > smallY) && (bigcZ < bigZ))
				{	// only bigY,smallZ corner within prim
				Overlap[prim][chkprim] = 20;
				}
			if(Overlap[prim][chkprim] != 0) continue;
			if((smallY > smallcY) && (smallY < bigcY)
					&& (bigZ > smallcZ) && (bigZ < bigcZ)
					&& (bigcY < bigY) && (smallcZ > smallZ))
				{	// only smallY,bigZ corner within prim
				Overlap[prim][chkprim] = 21;
				}
			if(Overlap[prim][chkprim] != 0) continue;
			if((bigY > smallcY) && (bigY < bigcY)
					&& (bigZ > smallcZ) && (bigZ < bigcZ)
					&& (smallcY > smallY) && (smallcZ > smallZ))
				{	// only bigY,bigZ corner within prim
				Overlap[prim][chkprim] = 22;
				}
			if(Overlap[prim][chkprim] != 0) continue;
...
for(register unsigned int prim = 1; prim <= NbPrimitives; ++prim)
	for(register unsigned int chkprim = prim+1;
														chkprim <= NbPrimitives; ++chkprim)
		{
		printf("prim: %d, XNorm: %lg, chkprim: %d, XNorm: %lg, Overlap: %d\n",
						prim, XNorm[prim], chkprim, XNorm[chkprim], Overlap[prim][chkprim]);
		}
...
if(rmprimFile == NULL)
	...
	if(NbRmPrims)
		{
		int tint;
		double rmXNorm[NbRmPrims+1], rmYNorm[NbRmPrims+1], rmZNorm[NbRmPrims+1];
		double rmXVert[NbRmPrims+1], rmYVert[NbRmPrims+1], rmZVert[NbRmPrims+1];
		for(int rmprim = 1; rmprim <= NbRmPrims; ++rmprim)
			...
			printf("rmprim: %d, rmXNorm: %lg, rmYNorm: %lg, rmZNorm: %lg, rmXVert: %lg, rmYVert: %lg, rmZVert: %lg\n",
							rmprim, rmXNorm[rmprim], rmYNorm[rmprim], rmZNorm[rmprim],
							rmXVert[rmprim], rmYVert[rmprim], rmZVert[rmprim]);
			}

		int remove[NbPrimitives + 1];
		for(int prim = 1; prim <= NbPrimitives; ++prim)	// check updated prim list
			{
			remove[prim] = 0;
			if(dbgFn)
				{
			...
			for(int rmprim = 1; rmprim <= NbRmPrims; ++rmprim)
				...
				if(dbgFn)
					{
					printf("rmprim: %d, rmXVertex: %lg, rmYVertex: %lg, rmZVertex: %lg\n",
				...
				if((fabs(fabs(XNorm[prim])-fabs(rmXNorm[rmprim])) <= MINDIST)
					...
					// For the time-being, we will assume that the planes to be removed
					// have their normals parallel to a given axis. So, we only check
					// that and remove the primitive if the distace along that axis match.
					// Possible pitfall => the primitives may be coplanar but
					// non-overlapping!
					if(fabs(fabs(XNorm[prim]) - 1.0) <= 1.0e-12)	// primitive || to YZ
						{
					...
					if(fabs(fabs(YNorm[prim]) - 1.0) <= 1.0e-12)	// primitive || to XZ
						{
					...
					if(fabs(fabs(ZNorm[prim]) - 1.0) <= 1.0e-12)	// primitive || to XY
						{
				...
				if(dbgFn)
					...
					printf("prim: %d, rmprim: %d, remove: %d\n",
									prim, rmprim, remove[prim]);
					}
				if(remove[prim] == 1) break;	// once removed, no point checking others
		...
		for(int prim = 1; prim <= NbPrimitives; ++prim)
			...
			for(int orgnlprim = 1; orgnlprim <= OrgnlNbPrimitives; ++orgnlprim)
				{
				if(OrgnlToEffPrim[orgnlprim][1] == prim)	// number updated for intrfc
					{
					orgnlNb = orgnlprim;
					break;
					}
			...
			if(remove[prim] == 1)
				...
				{	// keep this one in the updated list of primitives
			...
			}	// loop over primitives to remove the primitives tagged to be removed
		if(fprrm) fclose(fprrm);

		NbPrimitives -= NbRemoved;
		printf("Number of primitives removed: %d, Effective NbPrimitives: %d\n",
...
if(fignore == NULL)
	{
	printf("error opening IgnorePrims.info file in write mode ... returning\n");
	return(-1);
	}
...
for(int prim = 1; prim <= OrgnlNbPrimitives; ++prim)
	...
	fprintf(fignore, "%d %d %d\n",
										...
										OrgnlToEffPrim[prim][1],
										OrgnlToEffPrim[prim][2]);
	}
...
strcpy(NativeInFile, NativeOutDir); strcat(NativeInFile, "/neBEMNative.inp");
...
for(int prim = 1; prim <= NbPrimitives; ++prim)
	...
	strcpy(NativePrimFile, "Primitive"); strcat(NativePrimFile, strPrimNb);
	strcat(NativePrimFile, ".inp");

	fprintf(fNativeInFile, "#Input filename for primitive:\n");
	fprintf(fNativeInFile, "%s\n", NativePrimFile);

	strcpy(NativePrimFile, NativePrimDir); strcat(NativePrimFile, "/Primitive");
	strcat(NativePrimFile, strPrimNb); strcat(NativePrimFile, ".inp");
	...
	for(int vertex = 0; vertex < NbVertices[prim]; ++vertex)
		...
		fprintf(fNativePrim, "%lg %lg %lg\n",
													XVertex[prim][vertex], YVertex[prim][vertex],
													ZVertex[prim][vertex]);
		}	// for vertex
...
for(int volume = 0; volume <= VolMax; ++volume)
  ...
  neBEMVolumeDescription(volume, &shape, &material, &epsilon,
                          &potential, &charge, &boundarytype);
  ...
  strcat(NativeVolFile, "/Volume"); strcat(NativeVolFile, strVolNb);
...
if(NewModel && OptStorePrimitives)
	...
	if(OptUnformattedFile)
		{
		neBEMMessage("neBEMReadGeometry - unformatted write not inplemented yet.\n");
		return -1;
		}	// unformatted file
	}	// store primitives
...
NbSurfs = 0; NbWires = 0; NbElements = 0;
...
// have to be one among wire, right triangle and restangle. Any decomposition of
// arbitrary polygons into rectangles (as many as possible) and right triangles
// will have to be done earlier.
// All the counts have been incremented by one to be on the safe side!
// The additional memory can be minimized through a more careful computation
// of the required allocation for each type of primitive.
char MeshLogFile[256];

strcpy(MeshLogFile, MeshOutDir); strcat(MeshLogFile, "/MeshLog.out");
	...
	if(DebugLevel == 101)
		...
		if(NbVertices[prim] == 2)
			...
			printf("Primitive %d to be discretize into %d X %d elements.\n",
							prim, NbSurfSegX[prim], NbSurfSegZ[prim]);
			}
		}
	}
...
if(OptGnuplot)
	{
	char GnuFile[256];
	strcpy(GnuFile, MeshOutDir); strcat(GnuFile, "/GViewDir/gPrimView.gp");
	...
	strcpy(GnuFile, MeshOutDir); strcat(GnuFile, "/GViewDir/gElemView.gp");
	...
	strcpy(GnuFile, MeshOutDir); strcat(GnuFile, "/GViewDir/gMeshView.gp");
...
for(int prim = 1; prim <= NbPrimitives; ++prim)
  ...
  switch(PrimType[prim])
    ...
    case 4:     // rectangular surface
      ++NbSurfs;
      fstatus = SurfaceElements(prim, NbVertices[prim],
                                XVertex[prim], YVertex[prim], ZVertex[prim],
                                XNorm[prim], YNorm[prim], ZNorm[prim],
                                VolRef1[prim], VolRef2[prim],
																InterfaceType[prim], ApplPot[prim],
																ApplCh[prim], Lambda[prim],
    ...
    case 2:     // wire - a wire presumably has only 2 vertices
      ...
      fstatus = WireElements(prim, NbVertices[prim],
                                XVertex[prim], YVertex[prim], ZVertex[prim],
                                Radius[prim],
                                VolRef1[prim], VolRef2[prim],
																InterfaceType[prim], ApplPot[prim],
...
for(cntr1 = startcntr; cntr1 <= EleCntr; ++cntr1)
  ...
  for(cntr2 = cntr1+1; cntr2 <= EleCntr; ++cntr2)
	  ...
	  if(dist <= MINDIST)	// we need a linked-list here so that the overlapped
	    ...
	    // The element structure should also maintain information on the volumes
	    // that an element belongs to.
      ...
      printf("\tposition 1: (%g , %g , %g) micron,\n",
							1e6*pt1.X, 1e6*pt1.Y, 1e6*pt1.Z);
      printf("\tposition 2: (%g , %g , %g) micron.\n",
							1e6*pt2.X, 1e6*pt2.Y, 1e6*pt2.Z);
...
if(NewMesh && OptStoreElements)
	...
	if(OptUnformattedFile)
		{
		neBEMMessage("neBEMDiscretize - unformatted write not inplemented yet.\n");
		return -1;
		}	// unformatted file
	}	// store elements
...
if(KnChInpFile == NULL)
	{
	OptKnCh = 0;
	printf("neBEMKnCh.inp absent ... assuming absence of known charges ...\n");
	}
else
	{
	...
	if(!OptKnCh)
		printf("OptKnCh = 0 ... assuming no known charges ...\n");
	...
	if(OptKnCh)
		...
		if(NbPointsKnCh)
			...
			PointKnChArr = (PointKnCh *) malloc((NbPointsKnCh+1) * sizeof(PointKnCh));

			for(register int point = 0; point <= NbPointsKnCh; ++point)
			...
			char header[256]; fgets(header, 256, fptrPointKnChFile);	// header

			for(int point = 1; point <= NbPointsKnCh; ++point)
				{
				fscanf(fptrPointKnChFile, "%d %lg %lg %lg %lg\n",
					&PointKnChArr[point].Nb,
					&PointKnChArr[point].P.X,
					&PointKnChArr[point].P.Y,
					&PointKnChArr[point].P.Z,
		...
		if(NbLinesKnCh)
			{ // Lines
			if(debugFn) printf("No. of lines with known charges: %d\n", NbLinesKnCh);
			...
			LineKnChArr = (LineKnCh *) malloc((NbLinesKnCh+1) * sizeof(LineKnCh));

			for(register int line = 0; line <= NbLinesKnCh; ++line)
				{	// CHECK!!! ele limits start from 0, but all else from 1 to ...
			...
			char header[256]; fgets(header, 256, fptrLineKnChFile);	// header

			for(int line = 1; line <= NbLinesKnCh; ++line)
				{
				fscanf(fptrLineKnChFile, "%d %lg %lg %lg %lg %lg %lg %lg %lg\n",
					&LineKnChArr[line].Nb,
					&LineKnChArr[line].Start.X,
					&LineKnChArr[line].Start.Y,
					&LineKnChArr[line].Start.Z,
					&LineKnChArr[line].Stop.X,
					&LineKnChArr[line].Stop.Y,
					&LineKnChArr[line].Stop.Z,
					&LineKnChArr[line].Radius,
		...
		if(NbAreasKnCh)
			{ // Areas
			if(debugFn) printf("No. of areas with known charges: %d\n", NbAreasKnCh);
			...
			AreaKnChArr = (AreaKnCh *) malloc((NbAreasKnCh+1) * sizeof(AreaKnCh));

			for(register int area = 0; area <= NbAreasKnCh; ++area)
				{	// CHECK!!! ele limits start from 0, but all else from 1 to ...
			...
			char header[256]; fgets(header, 256, fptrAreaKnChFile);	// header

			for(int area=1; area <= NbAreasKnCh; ++area)
				{
				fscanf(fptrAreaKnChFile, "%d %d %le\n",
								&(AreaKnChArr+area-1)->Nb, &(AreaKnChArr+area-1)->NbVertices,
		...
		if(NbVolumesKnCh)
			...
			for(register int volume = 0; volume <= NbVolumesKnCh; ++volume)
				{	// CHECK!!! ele limits start from 0, but all else from 1 to ...
				VolumeKnChArr[volume].Nb = 0;
				VolumeKnChArr[volume].NbVertices = 0;
				for(int vert=1; vert <=(VolumeKnChArr+volume-1)->NbVertices; ++vert)
					{
			...
			char header[256]; fgets(header, 256, fptrVolumeKnChFile);	// header
...
if(ChargingUpInpFile == NULL)
	{
	printf("neBEMChargingUp.inp absent ... assuming no charging up effect ...\n");
	...
	if(OptChargingUp)
		...
		if(NbOfE <= 1)
			...
			for(register int ele = 0; ele <= NbElements; ++ele)
				{	// CHECK!!! ele limits start from 0, but all else from 1 to ...
				NbChUpEonEle[ele] = 0;
				}
			}
		...
		double xlbend, ylbend, zlbend, xend, yend, zend;	// lbend == Last But END
		Point3D ptintsct;	// each electron likely to have an intersection point
		for(int electron = 1; electron <= NbOfE; ++electron)
			{
			fscanf(fptrChargingUpEFile, "%c %d %d %lg %lg %lg %lg %lg %lg\n",
						&label, &vol, &enb, &xlbend, &xend, &ylbend, &yend, &zlbend, &zend);
			xlbend *= 0.01;
			xend *= 0.01;
			ylbend *= 0.01;
			yend *= 0.01;
			zlbend *= 0.01;
			zend *= 0.01;
			ptintsct.X = 0.0; ptintsct.Y = 0.0; ptintsct.Z = 0.0;	// initialize
			
			// find the parametric equation of this last segment
			// if xend < xlbend, swap the directions
			// This has not been mentioned as mandatory in Vince's book "Geometry
			// for Computer Graphics", but implied in the book "A Programmer's
			// Geometry"
			if(xend < xlbend) // not implemented now
				{
				}
			double lseg = (xend-xlbend)*(xend-xlbend) + (yend-ylbend)*(yend-ylbend)
										+ (zend-zlbend)*(zend-zlbend);
			lseg = sqrt(lseg);
			double xgrd = (xend-xlbend)/lseg;	// normalized direction vector
			...
			// Using the information on the last segment, find out which primitive
			// is pierced by it and at which point
			// From the intersection point, find out the element number on the
			// primitive in a local sense
			// Using the information (start and end elements of a given primitive)
			// identify the element in a global sense.
			// This approach should be lot more efficient than checking intersection
			// element by element.
			// The intersection point is computed following algorithm implemented in
			// a matlab code (plane_imp_line_par_int_3d.m)
			// Also check which primitive in the list is the closet to the end point

			double SumOfAngles;
			int PrimIntsctd = -1, EleIntsctd = -1; // intersected primitive & element
			int nearestprim = -1;	// absurd value
			double dist = 1.0e6, mindist = 1.0e6;	// absurdly high numbers
			for(int prim = 1; prim <= NbPrimitives; ++prim)	// check all primitives
				{
				if(InterfaceType[prim] != 4) continue;	// primitive not a dielectric

				int intersect=0, extrasect=1;	// worst of conditions
				int InPrim = 0, InEle = 0;
				if(debugFn)
					printf("prim: %d, mindist: %lg, nearestprim: %d\n",
									prim, mindist, nearestprim);

				// Use two nodes at a time to get two independent vectors on primitive	
				// Get cross-product of these two vector - normal to the plane
				// Note that the normal is already associated with the primitive
				// of type 3 and 4; 2 is wire and does not have any associated normal
				if((PrimType[prim] == 3) || (PrimType[prim] == 4))
					{
					if(debugFn)
						{
						...
						printf("vertex1: %lg, %lg, %lg\n",
								XVertex[prim][1], YVertex[prim][1], ZVertex[prim][1]);
						printf("vertex2: %lg, %lg, %lg\n",
								XVertex[prim][2], YVertex[prim][2], ZVertex[prim][2]);
						if(PrimType[prim] == 4)
							{
							printf("vertex3: %lg, %lg, %lg\n",
								XVertex[prim][3], YVertex[prim][3], ZVertex[prim][3]);
							}
					...
					if((prim == 1) && debugFn)
						printf("after prim == 1 check mindist: %lg, nearestprim: %d\n",
									 	mindist, nearestprim);

					// Point of intersection
					// Algo as on p62 (pdf 81) of Vince - Geometry for Computer Graphics
					// 1.5.13 Intersection of a line and a plane
					// Algorithm as implemented in plne_imp_line_par_int_3d.m
					// a (nx), b (ny), c (nz), d are a, b, c, d
					// vx, vy, vz are xgrd, ygrd and zgrd
					...
					double denom = a*xgrd + b*ygrd + c*zgrd;	// (vec)n.(vec)v; if 0, ||
					double tol = 1.0e-16;	// CHECK: -8 in original code; sizes small here
					...
					if(fabs(denom) < tol*norm1*norm2)	// line parallel to the plane
						{
						if(fabs(a*xlbend + b*ylbend + c*zlbend + d) <= 1.0e-16)
							{	// CHECK: was == 0.0 in original code
							intersect = 1; extrasect = 0;	// line ends on the plane
							ptintsct.X = xlbend;
							ptintsct.Y = ylbend;
							ptintsct.Z = zlbend;
							}
							...
							{
							intersect = 0; extrasect = 1;	// both wrong
							ptintsct.X = 0.0;	// Wrong to assign 0 values
							ptintsct.Y = 0.0;	// However, they are never going to be used
						...
						intersect = 1;
						double t = -(a*xlbend + b*ylbend + c*zlbend + d) / denom;

						// check whether t is less than the length of the segment
						// and in the correct direction
						// If not, then an extrapolated intersection is not of interest
						if((t < 0.0) || (fabs(t) > fabs(lseg)))	// wrong dirn or beyond end
							{
							extrasect = 1;
						...
						if(debugFn)
							...
							printf("intersection point: %lg, %lg, %lg\n",
											ptintsct.X, ptintsct.Y, ptintsct.Z);
							printf("t, lseg: %lg, %lg\n", t, lseg);
							printf("for an interesting intersection, lseg > t > 0.0 ...\n\n");
							fflush(stdout);
							}	// must intersect
						}	// if not parallel
					}	// if PrimType is 3 or 4
				else
					{	// this is a wire primitive - assume no charging up issues
					dist = -1.0;	// an absurd negative distance
					intersect = 0; extrasect = 0;
					continue;
				...
				// implicit assumption: the first primitive that gets pierced by the
				// ray is the one that we want. There can be other primitives that
				// are pierced by the same ray. So, this logic should be refined further
				if((intersect == 1) && (extrasect == 0))
					{
					// check whether the intersection point is within primitive polygon
					...
					if(debugFn)
						...
						printf("polynode0: %lg, %lg, %lg\n",
										polynode[0].X, polynode[0].Y, polynode[0].Z);
						printf("polynode1: %lg, %lg, %lg\n",
										polynode[1].X, polynode[1].Y, polynode[1].Z);
						printf("polynode2: %lg, %lg, %lg\n",
										polynode[2].X, polynode[2].Y, polynode[2].Z);
						if(nvert == 4)
							{
							printf("polynode3: %lg, %lg, %lg\n",
										polynode[3].X, polynode[3].Y, polynode[3].Z);
							}	
					...
					// Once identified, check in which element belonging to this primitive
					...
					if(InPrim)
						...
						for(int ele = ElementBgn[prim]; ele <= ElementEnd[prim]; ++ele)
							{
							nvert = 0;
							if((EleArr+ele-1)->G.Type == 3)
								nvert = 3;
							if((EleArr+ele-1)->G.Type == 4)
								nvert = 4;
							if(!nvert)
								{
							neBEMMessage("no vertex in element! ... neBEMKnownCharges ...\n");
								return -20;
								}
						...
						if(InEle)
							...
							{ 
							neBEMMessage("Element not identified ... neBEMKnownCharges\n");
							return -2;
							}
						}	// if InPrim
					}	// if intersection and no extrasection

				if((InPrim) && (intersect) && (!extrasect) && (InEle))	// all satisfied
					break;	// do not check any further primtive for this electron

				// If, after checking all the primitives, no interstion is found valid
				if(prim == (NbPrimitives))	// end of the list and no intersection
						{
						int nvert;
						Point3D polynode[4];
						int nearestele;
						double distele = 1.0e6, mindistele = 1.0e6;	// absurdly high value
						...
						for(int ele = ElementBgn[nearestprim];	// check all elements
							...
							if(!nvert)
								{
								neBEMMessage("no vertex element! ... neBEMKnownCharges ...\n");
								return -20;
								}
					...
					// ax + by + cz + d = 0 is the equation of the plane
								...
								*/
								...
								if(debugFn)
									...
									// printf("vector v: xgrd, ygrd, zgrd: %lg, %lg, %lg\n",
													// xgrd, ygrd, zgrd);
									printf(
									"distele: %lg, mindistele: %lg,from nearest ele origin: %d\n",
						...
						fprintf(fPtEChUpMap, "%d %lg %lg %lg %d %d %d %d\n",
											electron, ptintsct.X, ptintsct.Y, ptintsct.Z,
											PrimIntsctd, InPrim, EleIntsctd, InEle);
							// }	// if mindistele
			...
			if(debugFn) printf("writing file for checking electron positions\n");

			if(debugFn)	// check electron positions, volume primitives and elements
				{
				char elecposdbg[256], enbstr[10];
				sprintf(enbstr, "%d", electron);
				strcpy(elecposdbg, "/tmp/Electron"); strcat(elecposdbg, enbstr);
				strcat(elecposdbg, ".out");
				FILE *fepd = fopen(elecposdbg, "w");
				if(fepd == NULL)
					{
					printf("cannot open writable file to debug electron positions ...\n");
					printf("returning ...\n");
					return -111;
					}
				// write electron number, end, lbend, volume, primitive, elements, intxn
				fprintf(fepd, "#electron: %d %d\n", enb, electron);	// should print same
				...
				if(PrimIntsctd >= 1)
					...
					fprintf(fepd, "%lg %lg %lg\n",
		XVertex[PrimIntsctd][0], YVertex[PrimIntsctd][0], ZVertex[PrimIntsctd][0]);
					fprintf(fepd, "%lg %lg %lg\n",
		XVertex[PrimIntsctd][1], YVertex[PrimIntsctd][1], ZVertex[PrimIntsctd][1]);
					fprintf(fepd, "%lg %lg %lg\n",
		XVertex[PrimIntsctd][2], YVertex[PrimIntsctd][2], ZVertex[PrimIntsctd][2]);
					if(PrimType[PrimIntsctd] == 4)
						{
						fprintf(fepd, "%lg %lg %lg\n",
		XVertex[PrimIntsctd][3], YVertex[PrimIntsctd][3], ZVertex[PrimIntsctd][3]);
						}
					fprintf(fepd, "%lg %lg %lg\n",
		XVertex[PrimIntsctd][0], YVertex[PrimIntsctd][0], ZVertex[PrimIntsctd][0]);
					fprintf(fepd, "\n");
			...
			if(debugFn) printf("done writing file for checking electron positions\n");
		...
		for(int ele = 1; ele <= NbElements; ++ele)
			{
			(EleArr+ele-1)->Assigned += ChUpFactor*Q_E*NbChUpEonEle[ele]
																	/ (EleArr+ele-1)->G.dA;
			fprintf(fEleEChUpMap, "%d %lg %lg %lg %d %lg\n",
								...
								(EleArr+ele-1)->G.Origin.Y,
								(EleArr+ele-1)->G.Origin.Z,
								NbChUpEonEle[ele], (EleArr+ele-1)->Assigned);
			}
		fclose(fEleEChUpMap);
		...
		for(int ion = 1; ion <= NbOfI; ++ion)
			...
			fscanf(fptrChargingUpIFile, "%c %d %d %lg %lg %lg %lg %lg %lg\n",
						&label, &vol, &inb, &xlbend, &xend, &ylbend, &yend, &zlbend, &zend);
			xlbend *= 0.01;
			xend *= 0.01;
			ylbend *= 0.01;
			yend *= 0.01;
			...
			for(int prim = 1; prim <= NbPrimitives; ++prim)	// check all primitives
				...
				if((PrimType[prim] == 3) || (PrimType[prim] == 4))
					...
					// distance of the end point to this primitve is
					dist = (xend * XNorm[prim] + yend * YNorm[prim]
											+ zend * ZNorm[prim] + d)
												/
											sqrt(XNorm[prim]*XNorm[prim]
														+ YNorm[prim]*YNorm[prim]
					...
					double norm2 = sqrt(xgrd*xgrd + ygrd*ygrd + zgrd*zgrd);
					double denom = a*xgrd + b*ygrd + c*zgrd;	// (vec)n.(vec)v; if 0, ||
					double tol = 1.0e-12;	// CHECK: -8 in original code; sizes small here
					intersect = extrasect = 0;
					...
					if(fabs(denom) < tol*norm1*norm2)	// line parallel to the plane
						{
						if(a*xlbend + b*ylbend + c*zlbend + d == 0.0)	// CHECK == for float
							{
							intersect = 1; extrasect = 0;
							ptintsct.X = xlbend;
							ptintsct.Y = ylbend;
							ptintsct.Z = zlbend;
							}
				...
				if((intersect == 1) && (extrasect == 0))
					...
					for(int ele = ElementBgn[prim]; ele <= ElementEnd[prim]; ++ele)
						...
						if(InEle)
							...
							fprintf(fPtIChUpMap, "%d %lg %lg %lg %d %d %d %d\n",
											ion, ptintsct.X, ptintsct.Y, ptintsct.Z,
											prim, InPrim, ele, InEle);
					...
					if(InEle)
						...
						{ 
						neBEMMessage("Element cannot be identified ... neBEMKnownCharges\n");
						return -2;
						}
					...
					// ax + by + cz + d = 0 is the equation of the plane
							...
							*/
							...
							if(debugFn)
								...
								// printf("vector v: xgrd, ygrd, zgrd: %lg, %lg, %lg\n",
												// xgrd, ygrd, zgrd);
								printf("distele: %lg, mindist: %lg,  from nearest ele: %d\n",
												distele, mindistele, nearestele);
								fflush(stdout);
								}
			...
			if(debugFn)	// check ion positions, volume primitives and elements
				...
				strcpy(ionposdbg, "/tmp/Ion"); strcat(ionposdbg, inbstr);
				strcat(ionposdbg, ".out");
				FILE *fipd = fopen(ionposdbg, "w");
				if(fipd == NULL)
					{
					printf("cannot open writable file to debug ion positions ...\n");
					printf("returning ...\n");
					return -111;
					}
				// write electron number, end, lbend, volume, primitive, elements, intxn
		...
		for(int ele = 1; ele <= NbElements; ++ele)
			{
			(EleArr+ele-1)->Assigned += ChUpFactor*Q_I*NbChUpIonEle[ele]
																	/ (EleArr+ele-1)->G.dA;
			fprintf(fEleEIChUpMap, "%d %lg %lg %lg %d %lg\n",
								...
								(EleArr+ele-1)->G.Origin.Y,
								(EleArr+ele-1)->G.Origin.Z,
								NbChUpIonEle[ele], (EleArr+ele-1)->Assigned);
			}
...
if( (neBEMState == 5) || (neBEMState == 8) )
	...
	int fstatus;
	if(NewModel)
		{ // effectively, NewMesh = NewBC = NewPP = 1;
...
if(FailureCntr)
	{
	printf("neBEMSolve: Approximations were made while computing the influence coefficients.\n");
...
if(OptFastVol)
	...
	for(int block = 1; block <= FastVol.NbBlocks; ++block)
		...
		if(block == 1)
			...
			{
			if(MaxXCells < BlkNbXCells[block])
				MaxXCells = BlkNbXCells[block];
			if(MaxYCells < BlkNbYCells[block])
				MaxYCells = BlkNbYCells[block];
			if(MaxZCells < BlkNbZCells[block])
				MaxZCells = BlkNbZCells[block];
			}
	...
	if(OptStaggerFastVol)
		{
		/* Memory wastage!!! Improve as soon as possible. */
		FastStgPot = d4tensor(1, FastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
												1, MaxZCells+1);
		FastStgFX = d4tensor(1, FastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
												1, MaxZCells+1);
		FastStgFY = d4tensor(1, FastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
												1, MaxZCells+1);
		FastStgFZ = d4tensor(1, FastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
												1, MaxZCells+1);
		}	// if OptStaggerFastVol
  			...
  			for(int j = 1; j <= nbYCells+1; ++j)
    			...
    			for(int k = 1; k <= nbZCells+1; ++k)
      			...
      			fscanf(fFastVolPF,
              			...
              			&FastFY[block][i][j][k], &FastFZ[block][i][j][k]);
       			}  // loop k
     			}  // loop j
  			} // loop i
			}	// loop block
			...
			for(int block = 1; block <= FastVol.NbBlocks; ++block)
				...
				for(int i = 1; i <= nbXCells+1; ++i)
  				...
  				for(int j = 1; j <= nbYCells+1; ++j)
    				...
    				for(int k = 1; k <= nbZCells+1; ++k)
      				{
      				fscanf(fFastStgVolPF,
             						"%d\t%le\t%le\t%le\t%le\t%le\t%le\t%le\n",
             						&tmpblk, &xpt, &ypt, &zpt,
												&FastStgPot[block][i][j][k], &FastStgFX[block][i][j][k],
												&FastStgFY[block][i][j][k], &FastStgFZ[block][i][j][k]);
       				}  // loop k
     				}  // loop j
  				} // loop i
				}	// loop block
...
if(OptWtFldFastVol)	// allocate memory for weighting field fast volume variables
	{
	...
	/* Memory wastage!!! Improve as soon as possible. */
	WtFldFastPot = d4tensor(1, WtFldFastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
											1, MaxZCells+1);
	WtFldFastFX = d4tensor(1, WtFldFastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
											1, MaxZCells+1);
	WtFldFastFY = d4tensor(1, WtFldFastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
											1, MaxZCells+1);
	WtFldFastFZ = d4tensor(1, WtFldFastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
											1, MaxZCells+1);

	if(OptWtFldStaggerFastVol)
		{
		/* Memory wastage!!! Improve as soon as possible. */
		WtFldFastStgPot = d4tensor(1, WtFldFastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
												1, MaxZCells+1);
		WtFldFastStgFX = d4tensor(1, WtFldFastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
												1, MaxZCells+1);
		WtFldFastStgFY = d4tensor(1, WtFldFastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
												1, MaxZCells+1);
		WtFldFastStgFZ = d4tensor(1, WtFldFastVol.NbBlocks, 1, MaxXCells+1, 1, MaxYCells+1,
												1, MaxZCells+1);
		}	// if OptWtFldStaggerFastVol
	...
	if ((OptWtFldCreateFastPF) &&(!OptWtFldReadFastPF))	// reading overrides
		...
		neBEMMessage("neBEMSolve - Failure computing WtFldFastVolPF: not implemented");
		return -1;
  			...
  			for(int j = 1; j <= nbYCells+1; ++j)
    			...
    			for(int k = 1; k <= nbZCells+1; ++k)
      			...
      			fscanf(fFastVolPF,
              			"%d\t%le\t%le\t%le\t%le\t%le\t%le\t%le\n",
              			&tmpblk, &xpt, &ypt, &zpt,
										&WtFldFastPot[block][i][j][k], &WtFldFastFX[block][i][j][k],
              			&WtFldFastFY[block][i][j][k], &WtFldFastFZ[block][i][j][k]);
       			}  // loop k
     			}  // loop j
  			} // loop i
			}	// loop block
  				...
  				for(int j = 1; j <= nbYCells+1; ++j)
    				...
    				for(int k = 1; k <= nbZCells+1; ++k)
      				...
      				fscanf(fFastStgVolPF,
             						"%d\t%le\t%le\t%le\t%le\t%le\t%le\t%le\n",
             						&tmpblk, &xpt, &ypt, &zpt,
												&WtFldFastStgPot[block][i][j][k], &WtFldFastStgFX[block][i][j][k],
												&WtFldFastStgFY[block][i][j][k], &WtFldFastStgFZ[block][i][j][k]);
       				}  // loop k
     				}  // loop j
  				} // loop i
				}	// loop block
...
if(neBEMState < 7)
	{
  printf("neBEMPrepareWeightingField: Weighting computations only meaningful beyond neBEMState 7 ...\n");
  return -1;
  }
...
if (IdWtField >= MaxWtField)
	{
  printf("neBEMPrepareWeightingField: reached MaxWtField weighting fields.\n");
  return -1;
  }
...
if(!fstatus)	// estimate primitive related average wt field charge densities
	...
	for(int prim = 1; prim <= NbPrimitives; ++prim)
  	...
  	for(int ele = ElementBgn[prim];
                            	ele <= ElementEnd[prim]; ++ele)
    	{
    	area += (EleArr+ele-1)->G.dA;
    	AvWtChDen[IdWtField][prim] += WtFieldChDen[IdWtField][ele]
																		* (EleArr+ele-1)->G.dA;
    	}
...
for(int elem = 1; elem <= NbElements; ++elem)
  ...
  if(PeriodicInX[prim] || PeriodicInY[prim] || PeriodicInZ[prim])
   ...
   sumcharge += rptCnt * (EleArr+elem-1)->Solution * (EleArr+elem-1)->G.dA;
	}	// loop over elements
...
strcpy(subdir, ModelOutDir); strcat(subdir, "/Primitives/");
if(OptReuseDir)
	fstatus = CreateOrUseDir(subdir);
else
	fstatus = CreateDirOrQuit(subdir);

strcpy(subdir, MeshOutDir); strcat(subdir, "/Elements/");
if(OptReuseDir)
	fstatus = CreateOrUseDir(subdir);
else
	fstatus = CreateDirOrQuit(subdir);

strcpy(subdir, MeshOutDir); strcat(subdir, "/GViewDir/");
if(OptReuseDir)
	fstatus = CreateOrUseDir(subdir);
else
	fstatus = CreateDirOrQuit(subdir);
...
for(int prim=1; prim <= NbPrimitives; ++prim)
	...
	for(int vert=0; vert <NbVertices[prim]; ++vert)
		...
		fprintf(fStrPrm, "%le %le %le\n",
			XVertex[prim][vert], YVertex[prim][vert], ZVertex[prim][vert]);
		}	// vert loop
	...
	fprintf(fStrPrm, "%le %le %le %le %le\n",
		Epsilon1[prim], Epsilon2[prim], Lambda[prim], ApplPot[prim], ApplCh[prim]);
	
	fprintf(fStrPrm, "%d %d\n", VolRef1[prim], VolRef2[prim]);
	
	fprintf(fStrPrm, "%d %d %d\n",
					PeriodicTypeX[prim], PeriodicTypeY[prim], PeriodicTypeZ[prim]);
	fprintf(fStrPrm, "%d %d %d\n",
					PeriodicInX[prim], PeriodicInY[prim], PeriodicInZ[prim]);
	fprintf(fStrPrm, "%le %le %le\n",
					XPeriod[prim], YPeriod[prim], ZPeriod[prim]);
	fprintf(fStrPrm, "%le %le %le\n",
...
for(int ele=1; ele <= NbElements; ++ele)
  ...
  fprintf(fStrEle, "%d %d %d %d %d\n",
          (EleArr+ele-1)->DeviceNb, (EleArr+ele-1)->ComponentNb,
          (EleArr+ele-1)->PrimitiveNb, (EleArr+ele-1)->InterfaceId,
          (EleArr+ele-1)->Id);
  ...
  fprintf(fStrEle, "%d %le\n",
          (EleArr+ele-1)->E.Type, (EleArr+ele-1)->E.Lambda);
  fprintf(fStrEle, "%d %le %le %le %le\n",
          (EleArr+ele-1)->BC.NbOfBCs, (EleArr+ele-1)->BC.CollPt.X,
          (EleArr+ele-1)->BC.CollPt.Y, (EleArr+ele-1)->BC.CollPt.Z,
...
fprintf(fStrEle, "%d %d %d %d\n",
				NbPointsKnCh, NbLinesKnCh, NbAreasKnCh, NbVolumesKnCh);

for(int pt=1; pt <= NbPointsKnCh; ++pt)
	{
	fprintf(fStrEle, "%d %le\n", (PointKnChArr+pt-1)->Nb, (PointKnChArr+pt-1)->Assigned);
	fprintf(fStrEle, "%le %le %le\n",
					(PointKnChArr+pt-1)->P.X, (PointKnChArr+pt-1)->P.Y, (PointKnChArr+pt-1)->P.Z);
	}

for(int line=1; line <= NbLinesKnCh; ++line)
	{
	fprintf(fStrEle, "%d %le %le\n",
					(LineKnChArr+line-1)->Nb, (LineKnChArr+line-1)->Radius, (LineKnChArr+line-1)->Assigned);
	fprintf(fStrEle, "%le %le %le\n",
					(LineKnChArr+line-1)->Start.X, (LineKnChArr+line-1)->Start.Y,
					(LineKnChArr+line-1)->Start.Z);
	fprintf(fStrEle, "%le %le %le\n",
					(LineKnChArr+line-1)->Stop.X, (LineKnChArr+line-1)->Stop.Y,
...
for(int vol=1; vol <= NbVolumesKnCh; ++vol)
	{
	fprintf(fStrEle, "%d %d %le\n",
					(VolumeKnChArr+vol-1)->Nb, (VolumeKnChArr+vol-1)->NbVertices,
...
for (i=0;i<n;i++)
	...
	if (m1*m2 <= 1.0e-12)	// vetors of 1 micron - we may need to reduce further
	...
	// if(Sign(theta) != Sign(oldtheta))	// polygon either non-covex, or the
