...
double ****d4tensor(int nrl, int nrh, int ncl, int nch, int ndl, int ndh, int nsl, int nsh)
...
// allocate a double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh]
{
        ...
        // allocate rows and set pointers to them
        tt[nrl][ncl]=(double **) malloc((size_t)((nrow*ncol*ndep)*sizeof(double*)));
        if (!tt[nrl][ncl]) nrerror("allocation failure 3 in d4tensor()");
        tt[nrl][ncl] -= ndl;

        // allocate rows and set pointers to them
        tt[nrl][ncl][ndl]=(double *) malloc((size_t)((nrow*ncol*ndep*nsext)*sizeof(double)));
        if (!tt[nrl][ncl][ndl]) nrerror("allocation failure 4 in d4tensor()");
        tt[nrl][ncl][ndl] -= nsl;
        ...
        for(j=ncl+1;j<=nch;j++)
           ...
           ////////////////////////////////////////////////////////////////
        for(k=ndl+1;k<=ndh;k++)  tt[nrl][ncl][k]=tt[nrl][ncl][k-1]+nsext;   // (0;0;0~ndh).
        ...
        for(i=nrl+1;i<=nrh;i++)                                               //     (1~nrh;1~nch;0~ndh).
        {
      tt[i][ncl][ndl]=tt[i-1][ncl][ndl]+ncol*ndep*nsext;
          for(j=ncl+1;j<=nch;j++)
          {
...
// original version
double ****d4tensor(long nrl, long nrh, long ncl, long nch, long ndl, long ndh,long nd4l,long nd4h)

// allocate a double d4tensor with range t[nrl..nrh][ncl..nch][ndl..ndh][nd4l..nd4h]
{
   long i, j, k, nrow=nrh-nrl+1, ncol=nch-ncl+1, ndep=ndh-ndl+1,nd4th=nd4h-nd4l+1;
   double ****t;
   ...
   // allocate  rows and set pointers to them
   t[nrl][ncl][ndl]=(double *) malloc((size_t)((nrow*ncol*ndep*nd4th+NR_END)*sizeof(double)));
   if(!t[nrl][ncl][ndl]) nrerror("allocation failure 4 in d4tensor()");
   t[nrl][ncl][ndl] += NR_END;
   t[nrl][ncl][ndl] -= nd4l;
