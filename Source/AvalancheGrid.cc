...
bool AvalancheGrid::SnapToGrid(Grid& av, const double x, const double y,
    ...
    newNode.ix =indexX;newNode.iy =indexY; newNode.iz =indexZ;
    if(!GetParameters(newNode)){
        //TODO:Memory leak?
        if(m_debug) std::cerr << m_className << "::SnapToGrid: Could not retrieve parameters from sensor.\n";
        return false;
    }
    ...
    for (AvalancheNode& existingNode : m_activeNodes) {
        if(existingNode.ix == newNode.ix && existingNode.iy == newNode.iy && existingNode.iz == newNode.iz){
            alreadyExists = true;
...
void AvalancheGrid::NextAvalancheGridPoint(Grid& av) {
    ...
    for (AvalancheNode& node : m_activeNodes) {  // For every avalanche node
        ...
        node.ix += node.velNormal[0]; node.iy += node.velNormal[1]; node.iz += node.velNormal[2];
...
void AvalancheGrid::DeactivateNode(AvalancheNode& node){
    if (node.velNormal[2] != 0) {
        ...
        if ((node.velNormal[1] < 0 && node.iy == 0) ||
            (node.velNormal[1] > 0 && node.iy == m_avgrid.ysteps - 1)) node.active = false;
    } else {
        if ((node.velNormal[0] < 0 && node.ix == 0) ||
            (node.velNormal[0] > 0 && node.ix == m_avgrid.xsteps - 1))node.active = false;
    }
    ...
    int status;
    Medium* m = nullptr;
    m_sensor->ElectricField(m_avgrid.xgrid[node.ix],
                            m_avgrid.ygrid[node.iy], m_avgrid.zgrid[node.iz], e[0], e[1], e[2], v, m, status);
...
bool AvalancheGrid::GetParameters(AvalancheNode& node){
    ...
    m_sensor->ElectricField(
                            x, y,
                            z, e[0], e[1], e[2], v, m, status);
    
    if(m_debug)  std::cerr << m_className
        << "::GetParametersFromSensor::status = "
        <<status<<".\n";
    
    if(status == -5||status ==-6) return false; // If not inside a gas gap return false to terminate
    
    if (m_Townsend >=0){// If Townsend coef. is not set by user, take it from the sensor.
        node.townsend = m_Townsend;
    } else{
        m->ElectronTownsend(e[0], e[1], e[2], 0., 0., 0., node.townsend);
    }
    
    if (m_Attachment>=0){// If attachment coef. is not set by user, take it from the sensor.
        node.attachment = m_Attachment;
    }else{
        m->ElectronAttachment(e[0], e[1], e[2], 0., 0., 0., node.attachment);
    }
    
    if (m_Velocity > 0) {// If velocity is not set by user, take it from the sensor.
        ...
        if (vel != std::abs(vx) && vel != std::abs(vy) && vel != std::abs(vz)) return false;
    ...
    if(m_debug) std::cerr << m_className << "::GetParametersFromSensor::stepSize = "<<node.stepSize << "[cm].\n";
    
    if(m_debug) std::cerr << m_className << "::GetParametersFromSensor::velNormal = ("
        << node.velNormal[0] << ", " << node.velNormal[1] << ", " << node.velNormal[2]
        << ") [1].\n";
