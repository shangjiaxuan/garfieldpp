...
void AvalancheGrid::SetZGrid(Grid& av, const double ztop, const double zbottom, const int zsteps){
    ...
    // If diffusion is not enabled the z-coordinate grid is the only grid needed. So all the additional parameters will be set accordingly.
    if (!m_diffusion) {
...
int AvalancheGrid::GetAvalancheSize(double dx, const int nsize, const double alpha, const double eta){
    // Algorithm to get the size of the avalanche after it has propagated over a distance dx.
    ...
    // If the size is higher than 1e3 the central limit theorem will be used to describe the growth of the Townsend avalanche.
    if(nsize<1e3){
        ...
        for(int i = 0; i < nsize; i++){
            ...
            double s =dis(gen);
            // Condition to which the random number will be compared. If the number is smaller than the condition, nothing happens. Otherwise, the single electron will be attached or retrieve additional electrons from the gas.
...
void AvalancheGrid::SnapToGrid(Grid& av, const double x, const double z, const double v){
    ...
    // Get electron velocity from initial electrons, but this can be overwritten using the SetElectronVelocity function.
...
void  AvalancheGrid::NextAvalancheGridPoint(Grid& av){
    // This main function propagates the electrons and applies the avalanche statistics.
    int Nholder =0; // Holds the avalanche size before propagating it to the next point in the grid.
    
    av.run = false;
    
    for (int& pos : av.gridPosition){// For every avalanche position on the z-coorindate grid.

        if(pos==0) continue; // Check if it hqs reached the bottom resistive plate.
        ...
        for (int i = 0; i < av.xsteps; i++){
            ...
            if(m_diffusion){
                // Idem to the else part of this function, but with the additional step that after the new avalanche size is calculated the charges will be spread over the neighboring x-points following the normal distribution given by the transverse diffusion coefficient.
                double holdnsize =0.;
                ...
                int chargeRemaining =holdnsize; // Keeps charge of the amount of charge that is left over after it has spread to neighboring points. This will be the amount that will stay at the same x-coordinate grid point. This way no charge is lost during the diffusion step.
                ...
                // If the total avalanche size is smaller than the set saturation limit the GetAvalancheSize function is utilized to obtain the size after its propagation to the next z-coordinate grid point. Else, the size will be kept constant under the propagation.
                if(av.N<m_MaxSize){
                ...
                av.N+=av.n[pos-1][i]-Nholder; // Update total number of electrons.
            }
