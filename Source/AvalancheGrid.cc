...
void AvalancheGrid::SetGrid(const double xmin, const double xmax,const int xsteps,
    ...
    if (zmin >= zmax || zsteps <= 0||xmin > xmax || xsteps <= 0||ymin > ymax || ysteps <= 0){
        std::cerr << m_className << "::SetGrid:Error: Grid is not properly defined.\n";
        return;
    }
...
bool AvalancheGrid::SnapToGrid(Grid& av, const double x,const double y, const double z,const double v, const int n) {
    ...
    if(m_velNormal[0]!=0){
        indexX = m_velNormal[0]<0 ? floor((x-av.xgrid.front())/av.xStepSize) : ceil((x-av.xgrid.front())/av.xStepSize);
        ...
        indexY =  m_velNormal[1]<0 ? floor((y-av.ygrid.front())/av.yStepSize) : ceil((y-av.ygrid.front())/av.yStepSize);
        indexZ = round((z-av.zgrid.front())/av.zStepSize);
    }else{
        indexX = round((x-av.xgrid.front())/av.xStepSize);
        indexY = round((y-av.ygrid.front())/av.yStepSize);
        indexZ = m_velNormal[2]<0 ? floor((z-av.zgrid.front())/av.zStepSize) : ceil((z-av.zgrid.front())/av.zStepSize);
    }
    
    if(indexX<0 || indexX >= av.xsteps|| indexY<0 || indexY >= av.ysteps || indexZ<0 || indexZ >= av.zsteps) return false;
    ...
    if (m_debug) std::cerr << m_className << "::SnapToGrid: Snapped to (x,y,z) = ("
        <<x
        <<" -> "
        <<av.xgrid[indexX]
        <<", "
...
void AvalancheGrid::NextAvalancheGridPoint(Grid& av) {
    ...
    for (int& iz : av.gridPosition[0]) {  // For every avalanche position on the
        ...
        if ( (m_velNormal[2]<0 && iz == 0) || (m_velNormal[2]>0 && iz == av.zsteps-1) ){
            continue;
        } else{
            av.run = true;
        }
        ...
        for (int& iy : av.gridPosition[1]) {
            ...
            if ( (m_velNormal[1]<0 && iy == 0) || (m_velNormal[1]>0 && iy == av.ysteps-1) ){
                av.run = false;
                continue;
            } else{
                av.run = true;
            ...
            for (int& ix : av.gridPosition[2]) {
                ...
                if ( (m_velNormal[0]<0 && ix == 0) || (m_velNormal[0]>0 && ix == av.xsteps-1) ){
                    av.run = false;
                    continue;
                } else{
                    av.run = true;
                ...
                if (m_diffusion) {
                    ...
                    for (int j = av.transverseDiffusion.size() - 1; j >= 0; j--) {
                        ...
                        if (ix - jx < 0 || ix + jx > av.xsteps-1) continue;  // If in grid.
                        ...
                        if (j > 0) {  // For all neighboring points
                            ...
                            m_sensor->AddSignal(-(nxd + Nholder) / 2, av.time,
                                                ...
                                                av.xgrid[ix], av.ygrid[iy], av.zgrid[iz], av.xgrid[ix - j],
                                                av.ygrid[iy], av.zgrid[iz - 1], false, true);
                            ...
                            m_sensor->AddSignal(-(chargeRemaining + Nholder) / 2, av.time,
                                                av.time + av.zStepSize / av.velocity,
                                                av.xgrid[ix], av.ygrid[iy], av.zgrid[iz], av.xgrid[ix], av.ygrid[iy],
                    ...
                    av.N +=
                    av.n[iz + m_velNormal[2]][iy + m_velNormal[1]][ix + m_velNormal[0]] - Nholder;  // Update total number of electrons.
    ...
    if(m_velNormal[2]!=0){
        for (int& iz : av.gridPosition[0]) if ((m_velNormal[2]<0 && iz != 0) || (m_velNormal[2]>0 && iz != av.zsteps-1)) iz += m_velNormal[2];
    }else if(m_velNormal[1]!=0){
        for (int& iy : av.gridPosition[1]) if ((m_velNormal[1]<0 && iy != 0) || (m_velNormal[1]>0 && iy != av.ysteps-1)) iy += m_velNormal[1];
    }else{
        for (int& ix : av.gridPosition[2]) if ((m_velNormal[0]<0 && ix != 0) || (m_velNormal[0]>0 && ix != av.xsteps-1) ) ix += m_velNormal[0];
    }
...
void AvalancheGrid::DiffusionFactors(Grid& av) {
    ...
    while (factor > 1e-3) {  // 1e-3 is the precision cutoff.
        ...
        factor = (std::erf(x2/(m_DiffSigma*std::sqrt(2))) - std::erf(x1/(m_DiffSigma*std::sqrt(2))))/2;
        
        if(m_debug) std::cerr << m_className
            << "::DiffusionFactors::Transvers diffusion factor: " << factor
            << ", comparison: "
            << cdfunctop.Eval(0 - av.xStepSize / 2 + index * av.xStepSize)-cdfunctop.Eval(0 + av.xStepSize / 2 + index * av.xStepSize)
            << ", top: "
...
void AvalancheGrid::GetParametersFromSensor() {
    ...
    m_sensor->ElectricField(m_avgrid.xgrid[m_avgrid.xsteps / 2], m_avgrid.ygrid[m_avgrid.ysteps / 2], m_avgrid.zgrid[m_avgrid.zsteps / 2], e[0], e[1], e[2], v, m, status);
    
    if(m_Townsend==-1) m->ElectronTownsend(e[0], e[1], e[2], 0., 0., 0., m_Townsend);
    
    if(m_Attachment==-1) m->ElectronAttachment(e[0], e[1], e[2], 0., 0., 0., m_Attachment);
...
void AvalancheGrid::SortPositionVector() {
    ...
    for(int i = 0; i<3;i++){
        ...
        m_avgrid.gridPosition[i].erase(unique(m_avgrid.gridPosition[i].begin(), m_avgrid.gridPosition[i].end()),m_avgrid.gridPosition[i].end());
    }
