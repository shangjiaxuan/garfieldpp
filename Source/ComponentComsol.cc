...
bool ComponentComsol::SetDelayedWeightingField(const std::string& field, const std::string& label){
    ...
    while (std::getline(ffield, line)) {
        ...
        if ((Linecount + 1) % nPrint == 0) PrintProgress(double(Linecount + 1) / nNodes);
        Linecount++;
...
double ComponentComsol::DelayedWeightingPotential(const double xin, const double yin, const double zin,
    ...
    double dp0 = n0.dw[iw][i0] * t1 * (2 * t1 - 1) + n1.dw[iw][i0] * t2 * (2 * t2 - 1) +
    ...
    double dp1 = n0.dw[iw][i1] * t1 * (2 * t1 - 1) + n1.dw[iw][i1] * t2 * (2 * t2 - 1) +
