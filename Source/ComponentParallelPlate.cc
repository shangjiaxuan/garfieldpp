...
ComponentParallelPlate::ComponentParallelPlate() : 
...
void ComponentParallelPlate::Setup(double g,double b,double eps,double V, double sigma){
    ...
    m_g=g;m_b=b;m_eps=eps;m_V=V;m_sigma=sigma;
...
double ComponentParallelPlate::IntegrateField(const Electrode& el, int comp, const double X, const double Y, const double Z){
    ...
    switch(el.ind) {
            ...
            auto WFieldPixel = [](double* k, double* p) {
                ...
                switch(comp) {
                    case fieldcomponent::xcomp: {
                        intsol *= 1/(ky*cosh(G*K) * sinh(B *K) + EPS*ky*cosh(B*K) * sinh(G *K));
                        
                        intsol *= cos(ky*Y) * sin((kx*LX)/2)*sin((ky*LY)/2)*sin(kx*X)*sinh(K*(G - Z));
                        break;}
                    case fieldcomponent::ycomp:{
                        intsol *= 1/(kx*cosh(G*K) * sinh(B *K) + EPS*kx*cosh(B*K) * sinh(G *K));
                        intsol *= sin(ky*Y) * sin((kx*LX)/2)*sin((ky*LY)/2)*cos(kx*X)*sinh(K*(G - Z));
                        break;}
                    case fieldcomponent::zcomp:{
                        intsol *= 1/(ky*kx*cosh(G*K) * sinh(B *K) + EPS*ky*kx*cosh(B*K) * sinh(G *K));
                        intsol *= K*cos(ky*Y) * sin((kx*LX)/2)*sin((ky*LY)/2)*cos(kx*X)*cosh(K*(G - Z));
                        break;}
                }
                return intsol;
            };
        
                TF2 *fw = new TF2("WFieldPixel",WFieldPixel, 0, 10*m_g,0, 10*m_g, 10);
                
                fw->SetParameters(comp, X-el.xpos,Y-el.ypos,Z,m_Vw,m_eps,m_g,m_b,el.lx,el.ly); // Set a = 0, b = 1
        ...
        case structureelectrode::Strip:{
            ...
            auto WFieldStrip = [](double* k, double* p) {
                ...
                double intsol = 1/ ((cosh(G*kk)*sinh(B*kk)+EPS * cosh(B*kk) * sinh(G*kk)));
                switch(comp) {
                    case fieldcomponent::xcomp: {
            ...
            if(comp==ycomp){
                ...
                fw->SetParameters(comp, X-el.xpos,Z,m_Vw,m_eps,m_g,m_b,el.lx); // Set a = 0, b = 1
...
double ComponentParallelPlate::IntegratePromptPotential(const Electrode& el, const double X, const double Y, const double Z){
    ...
    switch(el.ind) {
        ...
        case structureelectrode::Pixel:{
            ...
            auto WPotentialPixel = [](double* k, double* p) {
                ...
                intsol *= cos(kx*X)*sin(kx*LX/2)*cos(ky*Y)*sin(ky*LY/2)* sinh(K*(G - Z))/(kx*ky* (sinh(B*K)*cosh(G*K)+EPS*sinh(G*K)*cosh(B*K)));
                
                return intsol;
                
            };
            
            TF2 *pw = new TF2("WPotentialPixel",WPotentialPixel, 0, 10*m_g,0, 10*m_g, 9);
            
            pw->SetParameters(X-el.xpos,Y-el.ypos,Z,m_Vw,m_eps,m_g,m_b,el.lx,el.ly); // Set a = 0, b = 1
        ...
        case structureelectrode::Strip:{
            ...
            auto WPotentialStrip = [](double* k, double* p) {
                ...
                double intsol = 1/ (kk * (cosh(G*kk)*sinh(B*kk)+EPS * cosh(B*kk) * sinh(G*kk)));
            ...
            pw->SetParameters(X-el.xpos,Z,m_Vw,m_eps,m_g,m_b,el.lx); // Set a = 0, b = 1
    ...
    switch(el.ind) {
        ...
        case structureelectrode::Pixel:{
            ...
            auto WPotentialPixel = [](double* k, double* p) {
                ...
                double K = std::sqrt(kx*kx + ky*ky);
                double tau = (1+cosh(G*K)*sinh(B*K)/(cosh(B*K)*sinh(G*K)))*(1/SIGMA); // Note to self: You dropt the eps) here for convenience.
                
                double intsol = 1/(kx*ky* (sinh(B*K)*cosh(G*K)+EPS*sinh(G*K)*cosh(B*K)));
                
                intsol *= cos(kx*X)*sin(kx*LX/2)*cos(ky*Y)*sin(ky*LY/2)* sinh(K*(G - Z))*tanh(B*K)*cosh(G*K)*(1-exp(-T/tau))/sinh(G*K);
                
                return intsol;
                
            };
            
            TF2 *pw = new TF2("WPotentialPixel",WPotentialPixel, 0, 10*m_g,0, 10*m_g, 11);
            
            pw->SetParameters(X-el.xpos,Y-el.ypos,Z,m_Vw,m_eps,m_g,m_b,el.lx,el.ly,t,m_sigma); // Set a = 0, b = 1
        ...
        case structureelectrode::Strip:{
            ...
            auto WPotentialStrip = [](double* k, double* p) {
                ...
                double tau = (EPS+cosh(G*kk)*sinh(B*kk)/(cosh(B*kk)*sinh(G*kk)))*(1/SIGMA); // Note to self: You dropt the eps) here for convenience.
                
                double intsol = 1/ (kk * (cosh(G*kk)*sinh(B*kk)+EPS * cosh(B*kk) * sinh(G*kk)));
                intsol *= (sin(kk*LX /2)*cos(kk*X)*sinh(kk*(G-Z))*cosh(G*kk)* tanh(B*kk) )*(1-exp(-T/tau))/sinh(G*kk);
            ...
            pw->SetParameters(X-el.xpos,Z,m_Vw,m_eps,m_g,m_b,el.lx,t,m_sigma); // Set a = 0, b = 1
...
double ComponentParallelPlate::WeightingPotential(const double x, const double y, const double z,
...
double ComponentParallelPlate::DelayedWeightingPotential(const double x, const double y,
...
void ComponentParallelPlate::DelayedWeightingField(const double x, const double y,
...
void ComponentParallelPlate::AddPixel(double x, double y, double lx_input, double ly_input, const std::string& label){
    const auto it = std::find(m_readout.cbegin(), m_readout.cend(), label);
...
void ComponentParallelPlate::AddStrip(double x, double lx_input, const std::string& label){
    const auto it = std::find(m_readout.cbegin(), m_readout.cend(), label);
    if (it == m_readout.end() && m_readout.size()>0){
        LOG("Label "<<label<<" already excists, please use an other lable.");
    } else{
...
Medium* ComponentParallelPlate::GetMedium(const double x, const double y, const double z){
