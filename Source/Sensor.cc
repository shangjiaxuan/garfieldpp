...
void Sensor::AddSignal(const double q, const double t0, const double t1,
    ...
    if (!m_useinducedcharge){
        ...
        int bin = int((t0 - m_tStart) / m_tStep);
        // Check if the starting time is outside the range
        if (bin < 0 || bin >= (int)m_nTimeBins) {
            if (m_debug) std::cout << "Bin " << bin << " out of range.\n";
            return;
...
double Sensor::GetCharge(const std::string& label, const unsigned int bin,const int comp) {
    if (m_nEvents == 0) return 0.;
    if (bin >= m_nTimeBins) return 0.;
    double sig = 0.;
    for (const auto& electrode : m_electrodes) {
...
double Sensor::GetSignal(const std::string& label, const unsigned int bin,const int comp) {
    if (m_nEvents == 0) return 0.;
    if (bin >= m_nTimeBins) return 0.;
    double sig = 0.;
    for (const auto& electrode : m_electrodes) {
...
double Sensor::GetPromptSignal(const std::string& label, const unsigned int bin) {
    if (m_nEvents == 0) return 0.;
    if (bin >= m_nTimeBins) return 0.;
    double sig = 0.;
    for (const auto& electrode : m_electrodes) {
...
double Sensor::GetDelayedSignal(const std::string& label, const unsigned int bin) {
    if (m_nEvents == 0) return 0.;
    if (bin >= m_nTimeBins) return 0.;
    double sig = 0.;
    for (const auto& electrode : m_electrodes) {
