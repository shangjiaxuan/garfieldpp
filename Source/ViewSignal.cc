...
void ViewSignal::PlotSignalTest(const std::string& label, const std::string setting, const bool same){
    ...
    const bool totalD = false;
    const bool electronT = false;
    const bool electronP = false;
    const bool electronD = false;
    const bool ionT = false;
    const bool ionP = false;
    const bool ionD = false;
    ...
    if (electronP) {
        ...
        for (unsigned int i = 0; i < nBins; ++i) {
            const double sig = m_sensor->GetElectronSignal(label, i)-m_sensor->GetDelayedElectronSignal(label, i);
            if (!std::isnan(sig) && std::abs(sig) < pres) {
    ...
    if (ionP) {
        ...
        for (unsigned int i = 0; i < nBins; ++i) {
            const double sig = m_sensor->GetIonSignal(label, i)-m_sensor->GetDelayedIonSignal(label, i);
            if (!std::isnan(sig) && std::abs(sig) < pres) {
    ...
    if (totalP) leg->AddEntry(m_hPromptSignal.get(), "Prompt induced signal", "l");
    if (totalD) leg->AddEntry(m_hDelayedSignal.get(), "Delayed induced signal", "l");
    
    if (electronT) leg->AddEntry(m_hSignalElectrons.get(), "Electron induced signal", "l");
    if (electronP) leg->AddEntry(m_hPromptElectrons.get(), "Electron prompt induced signal", "l");
    if (electronD) leg->AddEntry(m_hDelayedSignalElectrons.get(), "Electron delayed induced signal", "l");
    
    if (ionT) leg->AddEntry(m_hSignalIons.get(), "Ion induced signal", "l");
    if (ionP) leg->AddEntry(m_hPromptIons.get(), "Ion/hole prompt induced signal", "l");
    if (ionD) leg->AddEntry(m_hDelayedSignalIons.get(), "Ion/hole delayed induced signal", "l");
